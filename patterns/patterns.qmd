...
patterns2 <- t(patterns)

write.csv(patterns2, "patterns2.csv")

```

```{ojs}

mydata = FileAttachment("patterns2.csv").csv({ typed: true })

```

```{ojs}

points = mydata.flatMap(({ name, ...values }) =>
...
Plot.plot({
  ...
  symbol: {legend: true},
  marks: [
    Plot.dot(mydata, {x: "Item", y: "Class 1\nHigh behavioral control", fill: "black", r: 7, tip: true})
  ]
})
```
